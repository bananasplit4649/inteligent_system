...
def train_model(dataset_train:Dataset, dataset_val:Dataset,
                ...
                drop_rate:float, patience:int):
    model = SimpleModel(model_path,drop_rate=drop_rate).to(device)
    ...
    for epoch in range(epoch_num):
        ...
        for data in tqdm(data_loader):
            ...
            rating = data['rating'].reshape(batch_size,1).to(device)
...
if __name__ == "__main__":    
    ...
    parser.add_argument("-lr","--learing_rate", default=2.0e-5, type=float, help="learning rate")
    ...
    train = create_dataset(args.train_set_path,vocab)
    val = create_dataset(args.validation_set_path,vocab)
